...
grep -Fq 'GDC_GATEWAY_MAX_CONCURRENT_REQUESTS=4' "$ROOT/.env.example"
grep -Fq 'GDC_GATEWAY_MAX_CONCURRENT_PER_10000_WEIGHT=1000000000' "$ROOT/.env.example"
grep -Fq 'GDC_GATEWAY_MAX_INPUT_TOKENS_IN_FLIGHT=4096' "$ROOT/.env.example"
grep -Fq 'MAX_CONCURRENT_REQUESTS="${GDC_GATEWAY_MAX_CONCURRENT_REQUESTS:-4}"' "$ROOT/04-ops/create-gateway.sh"
grep -Fq 'MAX_INPUT_TOKENS_IN_FLIGHT="${GDC_GATEWAY_MAX_INPUT_TOKENS_IN_FLIGHT:-4096}"' "$ROOT/04-ops/create-gateway.sh"
...
grep -Fq 'OPS must use public chain state here' "$ROOT/04-ops/create-gateway.sh"
grep -Fq 'active_participant_count' "$ROOT/04-ops/create-gateway.sh"
...
grep -Fq 'header_up Host {$PUBLIC_HOST}' "$ROOT/04-ops/edge-node/Caddyfile"
if grep -Fq 'PUBLIC_EDGE_HOST' "$ROOT/04-ops/edge-node/Caddyfile"; then
  echo 'participant edge must not proxy proof traffic through the shared public edge' >&2
  exit 1
fi
...
grep -Fq 'gateway.join-client-key' "$ROOT/scripts/make-secrets.sh"
